...
- (NSArray*)issues_testcases
{
    ...
    return [NSArray
                arrayWithObjects:XVimMakeTestCase(text1, 12, 0, @"Vjd", issue_251_result, 16, 0), // Issue #251
                                 XVimMakeTestCase(text0, 0, 0, @"qadwpq", @"baaa bb ccc\n", 4, 0), // Issue #396
                                 XVimMakeTestCase(text1, 24, 0,
                                                  @":inoremap <lt>C-e> <lt>C-o>$<CR>i<Right><Right><Up><Up><C-e><ESC>",
                                                  text1, 10, 0), // Issue #416
                                 XVimMakeTestCase(text1, 20, 0, @"yyjp", issue_216_result, 36, 0),
                                 XVimMakeTestCase(text1, 4, 0, @"vll<D-x>ibbb<ESC>", text1, 6, 0), // Issue #429
                                 XVimMakeTestCase(text1, 4, 0, @"vll<D-c>", text1, 4,
                                                  3), // Issue #429 related (Cmd-c should not change selected range)
                                 XVimMakeTestCase(text0, 4, 0, @"vll<D-c><ESC>0vll<D-v>0", issue_429_result, 0,
                                                  0), // Issue #429 related (Cmd-v should overwrite the selection and
                                                      // exit from visual)

                                 // ^W should not yank
                                 XVimMakeTestCase(@"abc\n", 2, 0, @"cl<C-w><ESC>p", @"c\n", 0, 0),

                                 XVimMakeTestCase(issue_587, 5, 0, @"j", issue_587, 6,
                                                  0), // Issue #587 xvim_sb_init related

                                 ([XcodeTextPreferences() useTabsToIndent]) // check for tab/space indentation
                                             ? XVimMakeTestCase(text0, 0, 0, @"i<TAB><ESC>.", issue_606_result_tabs, 1,
                                                                0) // Issue #606. Repeating tab insertion crashes Xcode.
                                             : XVimMakeTestCase(
                                                           text0, 0, 0, @"i<TAB><ESC>.", issue_606_result_spaces, 7,
                                                           0), // Issue #606. Repeating tab insertion crashes Xcode.

                                 XVimMakeTestCase(issue_776_text, 0, 0, @"O<ESC>", issue_776_result, 0,
                                                  0), // Issue #776 crash
                                 ...
                                 XVimMakeTestCase(text0, 0, 0, @":nmap <lt>backspace> l<cr><bs><bs>", text0, 2,
                                                  0), // Issue #844 mapping <backspace>
                                 nil];
}
