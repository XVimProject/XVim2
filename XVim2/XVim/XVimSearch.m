...
- (XVimMotion*)motionForSearch:(NSString*)string forward:(BOOL)forward
{
    ...
    if (forward) {
        ...
        m = [XVimMotion motion:MOTION_SEARCH_FORWARD type:CHARWISE_EXCLUSIVE count:1];
    }
    else {
        XVim.instance.searcher.lastSearchBackword = YES;
        m = [XVimMotion motion:MOTION_SEARCH_BACKWARD type:CHARWISE_EXCLUSIVE count:1];
    }
    m.regex = string;
...
- (NSRange)executeSearch:(NSString*)searchCmd
                  display:(NSString*)displayString
...
{
    if (self.lastSearchCase == XVimSearchCaseInsensitive) {
        return YES;
    }
    if (self.lastSearchCase == XVimSearchCaseSensitive) {
        return NO;
    }
...
- (NSRange)searchCurrentWordFrom:(NSUInteger)from
                          forward:(BOOL)forward
                   matchWholeWord:(BOOL)wholeWord
                         inWindow:(XVimWindow*)window
{
    NSRange found = { NSNotFound, 0 };
    ...
    if (forward) {
        found = [self executeSearch:searchString
                            display:searchWord
                               from:from < wordEnd ? from : wordEnd
                           inWindow:window];
    }
    else {
        found = [self executeSearch:searchString
...
- (void)findForwardFrom:(NSUInteger)from to:(NSUInteger)to inWindow:(XVimWindow*)window
{
    ...
    if (self.confirmEach) {
        // TODO: [srcView scrollRectToVisible:[srcView xvim_boundingRectForGlyphIndex:self.lastFoundRange.location]];
...
{
    if (self.lastFoundRange.location == NSNotFound) {
        [window errorMessage:[NSString stringWithFormat:@"Number of occurrences replaced %ld",
                                                        (long)self.numReplacements]
                        ringBell:TRUE];
    }
}
@end
