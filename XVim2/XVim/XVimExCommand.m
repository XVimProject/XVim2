    ...
    switch (*parsing)
    {
        ...
        case '\'':
            ...
            if( '<' == mark ){
                ...
                XVimMark* m = [XVIM.marks markForName:[NSString stringWithFormat:@"%c", mark] forDocument:window.sourceView.documentURL.path];
                if(m!=nil) {
                    addr = m.line;
                    parsing+=2;
                }
...
- (void)make:(XVimExArg*)args inWindow:(XVimWindow*)window{
    NSWindow *activeWindow = [[NSApplication sharedApplication] mainWindow];
        NSEvent *keyPress = [NSEvent keyEventWithType:NSEventTypeKeyDown location:[NSEvent mouseLocation] modifierFlags:NSEventModifierFlagCommand timestamp:[[NSDate date] timeIntervalSince1970] windowNumber:[activeWindow windowNumber] context:[NSGraphicsContext graphicsContextWithWindow:activeWindow] characters:@"b" charactersIgnoringModifiers:@"b" isARepeat:NO keyCode:1];
...
- (void)writeMapsToConsoleWithFirstLetter:(NSString*)f forMapMode:(XVIM_MODE)mode{
    ...
    [map enumerateKeymaps:^(NSString *mapFrom, NSString *mapTo){
            // TODO: [XVIM writeToConsole:[NSString stringWithFormat:@"%@ %-10s %s",f, [mapFrom UTF8String], [mapTo UTF8String]] ];
    }];
}
...
- (void)run:(XVimExArg*)args inWindow:(XVimWindow*)window{
    NSWindow *activeWindow = [[NSApplication sharedApplication] mainWindow];
        NSEvent *keyPress = [NSEvent keyEventWithType:NSEventTypeKeyDown location:[NSEvent mouseLocation] modifierFlags:NSEventModifierFlagCommand timestamp:[[NSDate date] timeIntervalSince1970] windowNumber:[activeWindow windowNumber] context:[NSGraphicsContext graphicsContextWithWindow:activeWindow] characters:@"r" charactersIgnoringModifiers:@"r" isARepeat:NO keyCode:1];
    [[NSApplication sharedApplication] sendEvent:keyPress];
}
