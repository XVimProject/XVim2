...
- (BOOL)hasChild { return 0 != self.dict.count; }
...
{
    NSAssert([self.lastMappedKeys length] == 0 || [self.inputKeys hasPrefix:self.lastMappedKeys],
             @"lastMappedKeys must be a prefix of inputKeys! Must be a programming error");
    return [self.inputKeys substringWithRange:NSMakeRange(self.lastMappedKeys.length,
                                                          self.inputKeys.length - self.lastMappedKeys.length)];
}
...
- (void)unmapImpl:(NSMutableArray*)keystrokes atNode:(XVimKeymapNode*)node
{
    ...
    if (keystrokes.count != 0) {
        ...
        // This is the last node of the map( means most deep node of the keymap to unmap). Just delete target (this node
        // is deleted on upper node processing if thid node does not have child any more)
        node.target = nil;
    }
}
...
- (void)clear { [self.root.dict removeAllObjects]; }
...
- (XVimString*)mapKeys:(XVimString*)keys withContext:(XVimKeymapContext*)context forceFix:(BOOL)fix
{
    ...
    if (fix || !node.hasChild || unProcessedString.length != 0) {
        ...
        if (context.lastMappedNode != nil) {
            XVimString* newStr = [NSString stringWithFormat:@"%@%@%@", context.lastMappedNode.target,
                                                            context.unmappedKeys, unProcessedString];
            // Only when its remappable map and infinite loop guard not reached to 0
            //           and also when inputKeys is not prefix of target (because if inputKeys is prefix of the current
            //           target it means it goes to infinite loop)
...
- (void)enumerateKeymapsImpl:(XVimKeymapNode*)node
                         forKeys:(XVimString*)keys
