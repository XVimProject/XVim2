...
- (NSUInteger)wordsForward:(NSUInteger)index count:(NSUInteger)count option:(MOTION_OPTION)opt info:(XVimMotionInfo*)info
{
        ...
        for (pos = index + 1; pos <= [[self xvim_string] length]; ++pos) {
                ...
                else if ([self isNonblank:pos] && ([self rangePlaceholder:pos - 1].location != NSNotFound && rph.location == NSNotFound)) {
...
- (NSUInteger)wordsBackward:(NSUInteger)index count:(NSUInteger)count option:(MOTION_OPTION)opt
{
        ...
        for (;; --pos) {
                ...
                if (([self isNewline:pos - 1] && [self isBlankline:pos]) || (([self isWhitespaceOrNewline:pos - 1] && [self isNonblank:pos])) || (!(opt & BIGWORD) && [self isKeyword:pos - 1] != [self isKeyword:pos] && ![self isWhitespaceOrNewline:pos])) {
                        word_count++;
                }
        ...
        while (YES) {
             ...
             */
                        ...
                        while (ptr < maxIndex && !isNewline(characterAtIndex(h, ptr))) {
                                ...
                                if (characterAtIndex(h, ptr) == '"' && (ptr == sta || characterAtIndex(h, ptr - 1) != '\'' || characterAtIndex(h, ptr + 1) != '\'')) {
                                        ++do_quotes;
                                }

                                if (characterAtIndex(h, ptr) == '\\' && ptr + 1 < maxIndex && !isNewline(characterAtIndex(h, ptr + 1))) {
                                        ++ptr;
                                }
                                ++ptr;
                        }
                        ...
                        if (!cpo_match && initc != '\'' && findc != '\'') {
                                if (backwards) {
                                        ...
                                        if (col > 1) {
                                                ...
                                                else if (col > 2 && characterAtIndex(h, pos - 2) == '\\' && characterAtIndex(h, pos - 3) == '\'') {
                                                        pos -= 3;
                                                        break;
                                                }
                                        }
                                        ...
                                        if (pos < maxIndex && !isNewline(characterAtIndex(h, pos + 1))) {
                                                if (characterAtIndex(h, pos + 1) == '\\' && (pos < maxIndex - 2) && !isNewline(characterAtIndex(h, pos + 2)) && characterAtIndex(h, pos + 3) == '\'') {
                                                        pos += 3;
                                                        break;
                                                }
