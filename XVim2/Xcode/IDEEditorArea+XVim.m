...
{
    if (self == NSClassFromString(@"IDEEditorArea")) {
...
- (XVimCommandLine*)xvim_commandline{
    return [self extraDataForName:@"CommandLine"];
}
...
- (void)xvim__setEditorModeViewControllerWithPrimaryEditorContext:(id)arg1{
    ...
    XVimCommandLine *cmd = [self extraDataForName:@"CommandLine"];
    if( nil == cmd ){
        cmd = [[XVimCommandLine alloc] init];
        [self setExtraData:cmd forName:@"CommandLine"];
    }
